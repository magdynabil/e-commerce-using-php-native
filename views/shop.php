...
if ($_GET) {
    if (isset($_GET['subcategory'])) {
        if (is_numeric($_GET['subcategory'])) {
            ...
            $selectedDataResult = $productObject->getProductsBySubCategoryId();
        } else {
            header('location:errors/404.php');
        }
    } elseif (isset($_GET['category'])) {
        if (is_numeric($_GET['category'])) {
            $offerObject->setId($_GET['category']);
            $categoryObject->setId($_GET['category']);
            $selectedDataResult = $categoryObject->getProductsByCategoryId();
        } else {
            header('location:errors/404.php');
        }
        ...
        if (is_numeric($_GET['brand'])) {
            ...
            $selectedDataResult = $productObject->getProductsByBrandId();
        } else {
            header('location:errors/404.php');
        }
    }elseif (isset($_GET['offer'])) {
        if (is_numeric($_GET['offer'])) {
            $offerObject->setId($_GET['offer']);
            $selectedDataResult = $offerObject->getProductsByOfferId();
        } else {
            header('location:errors/404.php');
        }
    }else {
    ...
    $selectedDataResult = $productObject->selectData();
}
            ...
            <div class="col-lg-9">
                ...
                <div class="grid-list-product-wrapper">
                    <div class="product-grid product-view pb-20">
                        <div class="row">
                            <?php
                            if ($selectedDataResult) {
                                $products = $selectedDataResult->fetch_all(MYSQLI_ASSOC);
                                ...
                                ?>
                                        <li> <a data-toggle="collapse" data-parent="#faq" href="#shop-catigory-<?= $category['id'] ?>"><?= $category['name_en'] ?> <i class="ion-ios-arrow-down"></i></a>
                                            <ul id="shop-catigory-<?= $category['id'] ?>" class="panel-collapse collapse ">
                                                ...
                                                $subCategories = $subcategoryObject->getSubCategoriesByCategory();
                                                if ($subCategories) {
                                                ...
                                                <?php }
                                                    ...
                                                    echo "<div class=' alert alert-warning'>No subs </div>";
                                                } ?>
                                            </ul>
                                        </li>
                                <?php
